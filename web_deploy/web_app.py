...
def top_10_countries(df):
    ...
    top_10_c = country_total_amount_sorted.head(10)    
    return top_10_c
...
def update_output(contents, amount_range, frequency_range):
    if contents is not None:
        ...
        temp_Out = prepare_dataset(df)
        temp_n_Out = process_data(temp_Out)

        temp_Out.to_csv('Out.csv', sep=',', index=False, header=True)
        temp_n_Out.to_csv('No_Out.csv', sep=',', index=False, header=True)

        Out = create_finalmodel(temp_Out)
        n_Out = create_finalmodel(temp_n_Out)
